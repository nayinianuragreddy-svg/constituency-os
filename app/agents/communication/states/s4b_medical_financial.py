...
STATE_NAME='s4b_medical_financial'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_issue_type(text,["Yes","No"])
    ...
    return StateResult(next_state=next_state,reply_text='Detailed description?',field_collected=('medical_financial_assistance',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'medical_financial_assistance':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'medical_financial_assistance','value':result.normalized_value})])
