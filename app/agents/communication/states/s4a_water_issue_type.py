...
STATE_NAME = "s4a_water_issue_type"

def handle(conversation, message, context) -> StateResult:
    text = (message or "").strip()
    result = validate_issue_type(text,["No supply","Contamination","Pipeline break","Borewell","New connection"])
    ...
    return StateResult(
        ...
        field_collected=("water_issue_type", result.normalized_value),
        db_writes=[
            DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}),
            DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"water_issue_type": result.normalized_value}}}),
        ],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "water_issue_type", "value": result.normalized_value})],
    )
