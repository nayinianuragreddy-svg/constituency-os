...
STATE_NAME = "s4a_others_description"
...
def handle(conversation, message, context) -> StateResult:
    ...
    return StateResult(
        ...
        reply_text="Upload photo/document file_id or type skip.",
        field_collected=("others_description", result.normalized_value),
        db_writes=[
            DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}),
            DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"others_description": result.normalized_value}}}),
        ],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "others_description", "value": result.normalized_value})],
    )
