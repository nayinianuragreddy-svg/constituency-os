...
STATE_NAME = "s4a_electricity_description"
...
def handle(conversation, message, context) -> StateResult:
    ...
    if text.lower() in {"skip", "na", ""}:
        ...
        result = validate_free_text(text,0,1000)
    ...
    return StateResult(
        ...
        reply_text="Thanks. Please confirm complaint details.",
        field_collected=("electricity_description", result.normalized_value),
        db_writes=[DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}), DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"electricity_description": result.normalized_value}}})],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "electricity_description", "value": result.normalized_value})],
    )
