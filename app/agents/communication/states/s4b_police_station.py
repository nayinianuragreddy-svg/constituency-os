...
def handle(conversation,message,context)->StateResult:
    ...
    return StateResult(next_state=next_state,reply_text='FIR/Complaint number (or skip)?',field_collected=('police_station',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'police_station':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'police_station','value':result.normalized_value})])
