...
def handle(conversation, message, context) -> StateResult:
    ...
    return StateResult(
        ...
        reply_text="What is the severity?",
        field_collected=("rnb_location", result.normalized_value),
        db_writes=[
            DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}),
            DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"rnb_location": result.normalized_value}}}),
        ],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "rnb_location", "value": result.normalized_value})],
    )
