...
STATE_NAME='s4b_welfare_description'
...
def handle(conversation,message,context)->StateResult:
    ...
    return StateResult(next_state=next_state,reply_text='Thanks. Please confirm complaint details.',field_collected=('welfare_description',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'welfare_description':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'welfare_description','value':result.normalized_value})])
