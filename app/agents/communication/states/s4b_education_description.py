...
STATE_NAME='s4b_education_description'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_free_text(text,10,1000)
    ...
    return StateResult(next_state=next_state,reply_text='Thanks. Please confirm complaint details.',field_collected=('education_description',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'education_description':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'education_description','value':result.normalized_value})])
