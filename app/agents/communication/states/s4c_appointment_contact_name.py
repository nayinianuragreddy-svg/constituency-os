...
STATE_NAME='s4c_appointment_contact_name'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_free_text(text,2,80)
    ...
    return StateResult(next_state=next_state,reply_text='Contact person number?',field_collected=('appointment_contact_name',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'appointment_contact_name':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'appointment_contact_name','value':result.normalized_value})])
