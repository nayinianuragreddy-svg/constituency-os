...
STATE_NAME='s4b_medical_relation'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_issue_type(text,["Self","Family member","Neighbour"])
    ...
    return StateResult(next_state=next_state,reply_text='Nature of emergency?',field_collected=('medical_relation',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'medical_relation':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'medical_relation','value':result.normalized_value})])
