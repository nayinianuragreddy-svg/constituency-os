...
STATE_NAME = "s4a_electricity_households"
...
def handle(conversation, message, context) -> StateResult:
    ...
    return StateResult(
        ...
        reply_text="Share DISCOM complaint ref (or skip).",
        field_collected=("electricity_households_affected", result.normalized_value),
        db_writes=[
            DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}),
            DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"electricity_households_affected": result.normalized_value}}}),
        ],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "electricity_households_affected", "value": result.normalized_value})],
    )
