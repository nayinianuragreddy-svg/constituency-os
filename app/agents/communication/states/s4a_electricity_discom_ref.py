...
STATE_NAME = "s4a_electricity_discom_ref"
...
def handle(conversation, message, context) -> StateResult:
    ...
    return StateResult(
        ...
        field_collected=("electricity_discom_ref", result.normalized_value),
        db_writes=[DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}), DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"electricity_discom_ref": result.normalized_value}}})],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "electricity_discom_ref", "value": result.normalized_value})],
    )
