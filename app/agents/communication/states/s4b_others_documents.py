...
STATE_NAME='s4b_others_documents'
...
def handle(conversation,message,context)->StateResult:
    ...
    return StateResult(next_state=next_state,reply_text='Detailed description?',field_collected=('private_others_documents',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'private_others_documents':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'private_others_documents','value':result.normalized_value})])
