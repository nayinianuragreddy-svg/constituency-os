...
STATE_NAME = "s4a_rnb_description"
...
def handle(conversation, message, context) -> StateResult:
    ...
    return StateResult(
        ...
        field_collected=("rnb_description", result.normalized_value),
        db_writes=[DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}), DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"rnb_description": result.normalized_value}}})],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "rnb_description", "value": result.normalized_value})],
    )
