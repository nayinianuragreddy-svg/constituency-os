...
STATE_NAME='s4c_appointment_org_name'
...
def handle(conversation,message,context)->StateResult:
    ...
    return StateResult(next_state=next_state,reply_text='Purpose/Occasion?',field_collected=('appointment_org_name',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'appointment_org_name':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'appointment_org_name','value':result.normalized_value})])
