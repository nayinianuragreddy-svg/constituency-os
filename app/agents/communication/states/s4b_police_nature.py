...
STATE_NAME='s4b_police_nature'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_issue_type(text,["FIR not registered","Harassment","False case","Property dispute","Threat","Other"])
    ...
    return StateResult(next_state=next_state,reply_text='Incident date (DD/MM/YYYY)?',field_collected=('police_nature',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'police_nature':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'police_nature','value':result.normalized_value})])
