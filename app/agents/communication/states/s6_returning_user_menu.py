...
STATE_NAME = "s6_returning_user_menu"

def handle(conversation, message, context) -> StateResult:
    text=(message or "").strip()
    if STATE_NAME=="s1_greet":
        return StateResult(next_state="s1_language_select", reply_text="Welcome back. Choose an option.", reply_buttons=[Button("Telugu","te"),Button("Hindi","hi"),Button("English","en")])
    if STATE_NAME in {"s2_register_done","s5_acknowledgement"}:
        return StateResult(next_state="s6_returning_user_menu", reply_text="Welcome back. Choose an option.")
    ...
    return StateResult(next_state=next_state, reply_text="Welcome back. Choose an option.", field_collected=("returning_user_menu",v.normalized_value), db_writes=[DbWrite("upsert","citizens",values={"returning_user_menu":v.normalized_value})], agent_actions_to_log=[ActionLog("field.collected",{"field":"returning_user_menu"})])
