...
STATE_NAME='s4b_welfare_category'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_issue_type(text,["Pension","Housing","Ration card","Caste certificate","Women scheme","SC/ST scheme","Other"])
    ...
    return StateResult(next_state=next_state,reply_text='Scheme name (or skip)?',field_collected=('welfare_category',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'welfare_category':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'welfare_category','value':result.normalized_value})])
