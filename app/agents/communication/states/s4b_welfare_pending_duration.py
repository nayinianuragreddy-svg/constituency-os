...
STATE_NAME='s4b_welfare_pending_duration'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_free_text(text,3,120)
    ...
    return StateResult(next_state=next_state,reply_text='Detailed description?',field_collected=('welfare_pending_duration',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'welfare_pending_duration':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'welfare_pending_duration','value':result.normalized_value})])
