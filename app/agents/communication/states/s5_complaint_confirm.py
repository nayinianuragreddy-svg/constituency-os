...
STATE_NAME = "s5_complaint_confirm"

def handle(conversation, message, context) -> StateResult:
    text=(message or "").strip()
    if STATE_NAME=="s1_greet":
        return StateResult(next_state="s1_language_select", reply_text="Please confirm complaint details.", reply_buttons=[Button("Telugu","te"),Button("Hindi","hi"),Button("English","en")])
    if STATE_NAME in {"s2_register_done","s5_acknowledgement"}:
        return StateResult(next_state="s5_complaint_confirm", reply_text="Please confirm complaint details.")
    ...
    return StateResult(next_state=next_state, reply_text="Please confirm complaint details.", field_collected=("complaint_confirm",v.normalized_value), db_writes=[DbWrite("upsert","citizens",values={"complaint_confirm":v.normalized_value})], agent_actions_to_log=[ActionLog("field.collected",{"field":"complaint_confirm"})])
