...
STATE_NAME = "s2_register_geo"

def handle(conversation, message, context) -> StateResult:
    text=(message or "").strip()
    if STATE_NAME=="s1_greet":
        return StateResult(next_state="s1_language_select", reply_text="Please confirm your details below.", reply_buttons=[Button("Telugu","te"),Button("Hindi","hi"),Button("English","en")])
    if STATE_NAME in {"s2_register_done","s5_acknowledgement"}:
        return StateResult(next_state="s2_register_confirm", reply_text="Please confirm your details below.")
    ...
    return StateResult(next_state=next_state, reply_text="Please confirm your details below.", field_collected=("geo",v.normalized_value), db_writes=[DbWrite("upsert","citizens",values={"geo":v.normalized_value})], agent_actions_to_log=[ActionLog("field.collected",{"field":"geo"})])
