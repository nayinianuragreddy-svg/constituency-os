...
STATE_NAME = "s4a_electricity_location"
...
def handle(conversation, message, context) -> StateResult:
    ...
    return StateResult(
        ...
        field_collected=("electricity_location", result.normalized_value),
        db_writes=[
            DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}),
            DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"electricity_location": result.normalized_value}}}),
        ],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "electricity_location", "value": result.normalized_value})],
    )
