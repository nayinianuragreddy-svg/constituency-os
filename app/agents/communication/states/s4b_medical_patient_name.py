...
STATE_NAME='s4b_medical_patient_name'
...
def handle(conversation,message,context)->StateResult:
    ...
    return StateResult(next_state=next_state,reply_text='Patient age?',field_collected=('medical_patient_name',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'medical_patient_name':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'medical_patient_name','value':result.normalized_value})])
