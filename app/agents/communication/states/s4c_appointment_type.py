...
STATE_NAME='s4c_appointment_type'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_issue_type(text,["Meeting","Event invitation","Felicitation","Programme"])
    ...
    return StateResult(next_state=next_state,reply_text='Organisation/Individual name?',field_collected=('appointment_type',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'appointment_type':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'appointment_type','value':result.normalized_value})])
