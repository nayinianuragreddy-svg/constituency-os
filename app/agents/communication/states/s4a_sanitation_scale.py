...
from app.agents.communication.validators import validate_scale
from app.agents.communication.transitions import resolve_next

STATE_NAME = "s4a_sanitation_scale"

def handle(conversation, message, context) -> StateResult:
    text = (message or "").strip()
    result = validate_scale(text)
    ...
    return StateResult(
        ...
        field_collected=("sanitation_scale", result.normalized_value),
        db_writes=[
            DbWrite("update", "citizen_conversations", values={"invalid_attempts_in_state": 0}),
            DbWrite("update", "citizen_conversations", values={"draft_payload": {"$merge": {"sanitation_scale": result.normalized_value}}}),
        ],
        agent_actions_to_log=[ActionLog("field.collected", {"field": "sanitation_scale", "value": result.normalized_value})],
    )
