...
STATE_NAME='s4b_revenue_description'
...
def handle(conversation,message,context)->StateResult:
    ...
    return StateResult(next_state=next_state,reply_text='Thanks. Please confirm complaint details.',field_collected=('revenue_description',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'revenue_description':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'revenue_description','value':result.normalized_value})])
