...
STATE_NAME='s4b_education_ref_number'
...
def handle(conversation,message,context)->StateResult:
    ...
    if text.lower() in {'skip','na',''}:
        ...
        result=validate_free_text(text,0,120)
    ...
    return StateResult(next_state=next_state,reply_text='Detailed description?',field_collected=('education_ref_number',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'education_ref_number':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'education_ref_number','value':result.normalized_value})])
