...
from app.agents.communication.validators import validate_attendees
from app.agents.communication.transitions import resolve_next

STATE_NAME='s4c_appointment_attendees'
...
def handle(conversation,message,context)->StateResult:
    ...
    if text.lower() in {'skip','na',''}:
        ...
        result=validate_attendees(text)
    ...
    return StateResult(next_state=next_state,reply_text='Contact person name?',field_collected=('appointment_attendees',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'appointment_attendees':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'appointment_attendees','value':result.normalized_value})])
