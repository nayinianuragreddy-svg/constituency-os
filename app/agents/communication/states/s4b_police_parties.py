...
STATE_NAME='s4b_police_parties'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_free_text(text,5,300)
    ...
    return StateResult(next_state=next_state,reply_text='Urgency level?',field_collected=('police_parties',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'police_parties':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'police_parties','value':result.normalized_value})])
