...
STATE_NAME='s4b_revenue_village_mandal'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_free_text(text,2,120)
    ...
    return StateResult(next_state=next_state,reply_text='Status of issue?',field_collected=('revenue_village_mandal',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'revenue_village_mandal':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'revenue_village_mandal','value':result.normalized_value})])
