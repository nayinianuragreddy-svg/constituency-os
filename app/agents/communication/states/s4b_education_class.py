...
STATE_NAME='s4b_education_class'

def handle(conversation,message,context)->StateResult:
    text=(message or '').strip()
    result=validate_free_text(text,1,80)
    ...
    return StateResult(next_state=next_state,reply_text='Status?',field_collected=('education_class',result.normalized_value),db_writes=[DbWrite('update','citizen_conversations',values={'invalid_attempts_in_state':0}),DbWrite('update','citizen_conversations',values={'draft_payload':{'$merge':{'education_class':result.normalized_value}}})],agent_actions_to_log=[ActionLog('field.collected',{'field':'education_class','value':result.normalized_value})])
